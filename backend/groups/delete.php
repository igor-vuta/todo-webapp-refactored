...
if (!$stmtCheck->fetch()) {
    ...
    echo json_encode(['error' => 'Only the owner can delete the group']);
    exit;
}

$stmt = $pdo->prepare('DELETE FROM `groups` WHERE id = :group_id');

try {
    $stmt->execute(['group_id' => $group_id]);
} catch (PDOException $e) {
    http_response_code(500);
    echo json_encode(['error' => 'Failed to delete group', 'message' => $e->getMessage()]);
    exit;
}
